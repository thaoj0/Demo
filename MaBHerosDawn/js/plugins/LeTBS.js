...
BattleManagerTBS.getCellNear = function (center, destCell, maxRange) {
    ...
    this._easystar.setAcceptableTiles([0]);
    this._easystar.enableSync();
    if (BattleManagerTBS.isCellReachable(destCell, maxRange, center, reachables))
        return destCell;
    do {
        ...
        for (var i = 0; i < scope.length; i++) {
            var cell = scope[i];
            if (BattleManagerTBS.isCellReachable(cell, maxRange, center, reachables)) {
                foundCell = cell;
                break;
            }
        }
...
BattleManagerTBS.getCellAway = function (center, destCell, maxRange) {
    ...
    var scope = this.makeCircleScope(center, range, null, {}).filter(function (cell) {
        return BattleManagerTBS.isCellReachable(cell, maxRange, center, []);
    });
...
TBSSequenceManager.prototype.isRunning = function () {
    return this._sequence.table.length > 0;
};
...
TBSProjectileSprite.prototype.initialize = function (id, start, end, effects) {
    Sprite_Base.prototype.initialize.call(this);
    this._start = start;
    this._end = end;
    this._trajectory = [];
...
TBSEntity.prototype.hasNoCorpse = function () {
    return this.battler().hasLeTBSTag("hasNoCorpse");
};
...
TBSEntity_Sprite.prototype.processBitmapsConfig = function (info, hue) {
    var pose = info[0];
    this._filenameID = this.filenameID();
    var filename = this._filenameID + info[1];
    this._poses.push(pose);
    if (this._entity.hasBlankSprite() || (pose === "dead" && this._entity.hasNoCorpse())) {
...
TBSEntity_Sprite.prototype.getConfig = function () {
    var battler = this._battler;
    var def = battler.isActor() ? battler.name() : battler.originalName();
    var id = battler.getLeTBSTagStringValue("spriteConfig", def);
...
TBSEntity_Sprite.prototype.updateBalloon = function () {
    if (this._balloonSprite) {
        this._balloonSprite.x = this.x + this.width / 2;
...
Game_Interpreter.prototype.updateWaitMode = function () {
    if (this._leTBSCommandFlow) {
        ...
        if (!waiting) {
            ...
            this._leTBSCommandFlow.onEnd = function () { };
        }
        return waiting;
    } else {
...
Game_Interpreter.prototype.interpretLeTBS_ForceMove = function (args) {
    ...
    if (entity && entity.battler().isAlive() && cell) {
        ...
        if (!entity.isMoving())
            return false;
        /*else
            BattleManagerTBS.setCursorCell(entity._movePath[0]);*/
    } else {
        return false;
    }
    return {
